...
#define SETPWM2(pwm) 	{ CCP5CONbits.CCP5M = 0b1100; CCPR5L = pwm; CCP5CONbits.DC5B = 0;}
...
//	APFCON1 = 0x00;				// PWM2 output on pin RC3
	...
	TRISCbits.TRISC2 = 1;			// disable PWM output until set up
	TRISCbits.TRISC7 = 1;			// disable PWM output until set up
	TRISBbits.TRISB5 = 1;			// disable PWM output until set up
	TRISCbits.TRISC6 = 1;			// disable PWM output until set up
	
	// PWM1 initialization -- initially off
	CCP5CONbits.CCP5M = 0b1100;		// PWM mode
	CCP5CONbits.DC5B = 0b00;		// Lowest 2 bits of PWM duty cycle
	CCPR5L = 0x00;				// Upper 8 bits of PWM duty cycle (50% duty cycle)
	CCPTMRS0bits.C5TSEL = 0b00;		// Use Timer2 for this PWM
	...
	TRISCbits.TRISC2 = 0;			// disable PWM output until set up
	TRISCbits.TRISC7 = 0;			// disable PWM output until set up
	TRISBbits.TRISB5 = 0;			// disable PWM output until set up
	TRISCbits.TRISC6 = 0;			// disable PWM output until set up
